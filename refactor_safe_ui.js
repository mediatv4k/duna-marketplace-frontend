...
// 1. Inject the options at the start of the !isSlotMode block
const globalVariantsRegex = /\{\!isSlotMode && availableGroups\.map\(\(group: any, gIdx: number\) => \(/;

const injection = `      {viewMode === 'options' && (
...
code = code.replace(globalVariantsRegex, injection);

// 2. Hide the old "Panel Colaborativo" entirely.
// Find the exact block in the footer using a non-greedy regex.
const panelRegex = /\{\s*\/\*\s*(?:───|──|🧑‍🤝‍🧑)\s*Panel Colaborativo[\s\S]*?\{hasVariants && showComboPanel && comboRoomId && \([\s\S]*?\n\s*\}\)/g;
code = code.replace(panelRegex, `{(false) && (null)}`);

// If the regex above failed because it didn't match the closing brace perfectly, we'll just replace the start condition
...
// 4. TS Error on handleSingleSelect and handleOptionCount in my new code
code = code.replace(/handleSingleSelect\(/g, 'handleGlobalSingleSelect(');
code = code.replace(/handleOptionCount\(/g, 'handleGlobalOptionQuantityChange(');
