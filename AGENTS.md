...
- **Numero de pedido visible = `order_number` y botones del pie acotados en desktop (2026-09-25; `CheckoutModal.tsx`, `OrderTrackingModal.tsx`, `order/[orderId]/timeline/page.tsx`, `MasterProductModal.tsx`)**: (2.1) motivo: requisito de Adonis: el cliente y el comercio ven el correlativo comercial (`order_number`), no el id primario de la base de datos. Solucion: `CheckoutModal` toma `response.data.order_number` (o `orderNumber`) y lo pasa en `onFinalizeOrder({ orderNumber })` (campo opcional nuevo de `SubmittedOrderPayload`; se persiste en `last_active_order` solo si es un correlativo real, el id nunca se guarda como numero). Las cabeceras de las confirmaciones dicen "Confirmacion De Orden · N° X" / "Orden registrada · N° X" (`order_number || id`: el id solo como respaldo, segun el requisito). `OrderTrackingModal` y `/order/[orderId]/timeline` calculan `displayId = remote.order_number || orderData.orderNumber || id (respaldo)` y NO muestran el id mientras carga la primera consulta (la cabecera dice "Orden" y luego "Orden #N", para que no aparezca un instante el id que despues cambia). Comanda POS ("COMANDA N° X") y Recibo ("#X") usan el mismo valor. El id primario sigue siendo el que se usa para consultar (`getOrderPublic`), para `last_active_order_id` y para la URL `/order/{id}/timeline`. Pendiente de confirmar con la primera orden real: si la respuesta de compra NO trae `order_number` (forma no verificada en vivo), la confirmacion muestra el id como respaldo y el seguimiento cambia solo al correlativo del endpoint publico (que si lo trae, verificado con la orden #1620). (2.2) motivo: en `md:` los botones principales del pie del modal de producto/sala se estiraban al 100 %. Solucion: "Crear Sala y Enviar a WhatsApp", "Compartir de nuevo por WhatsApp" y "Proceder al Pago y Despacho" pasan a `md:w-auto md:px-8`, alineados a la derecha; en la sala (`comboRoom`) el pie es una sola fila en md+ (`md:flex md:flex-row-reverse md:items-center md:gap-6`): Total a Pagar a la izquierda y el boton a la derecha; en movil (<md) el orden apilado y el ancho completo no cambian. Medido en navegador: a 1366/1024/768 px el boton mide ~45 % del pie (24 px de margen derecho) y el Total queda a 24 px del borde izquierdo en la misma fila; a 360 px el boton sigue a todo el ancho sobre el Total. Sin hooks nuevos y sin cambios en `orderData`. Detalle en la bitacora tecnica.
