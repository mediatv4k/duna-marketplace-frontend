...
- **Carrusel promocional del hero (2026-09-22, Fase 11, `page.tsx` + `HeroBannerCarousel.tsx`):** nuevo componente montado justo debajo del `<header>` y antes de `PromotionsCarousel`, con tres diapositivas fijas de marketing (no vienen del backend, son banners de campaña de D'una, no de un comercio ni producto real — por eso no aplica la regla de "datos 100% reales" del catálogo). Slider nativo sin librería (mismo criterio que el autoplay de `PromotionsCarousel`: `setInterval` + `transform: translateX`, sin swiper/embla, para no sumar una dependencia): rota sola cada 4.5 s, con dots abajo (clic salta directo a esa diapositiva) y se pausa con el mouse encima o al tocar. Proporción panorámica fija `aspect-[3.2/1]` con `object-cover` — coincide exactamente con la resolución real de los tres banners (1280×400 px; verificado leyendo el marcador SOF de cada archivo: son JPEG reales pese a la extensión `.png` con la que llegaron a `public/images/`, el navegador los sirve igual porque detecta el contenido, no la extensión). Diapositivas: `banner-commer.png` → enlace externo `https://tr.ee/aJWg3IoL3q` (`target="_blank" rel="noopener noreferrer"`); `banner-delivery.png` → enlace externo `https://tr.ee/O553DC8j5Q`; `banner-cliente.png` → sin enlace, dispara `document.getElementById('categorias-tiendas')?.scrollIntoView({behavior:'smooth'})` (el `<section>` de "Categorías"/"Tiendas" ganó `id="categorias-tiendas"` y `scroll-mt-24` para no quedar tapado por el header sticky al llegar). Verificado en navegador (390 px): imagen y proporción correctas (358×112, ratio 3.20 exacto, sin scroll horizontal), 3 dots, el enlace externo trae el `href`/`rel` correctos, clic en un dot salta al slide exacto (`translateX(-200%)` al ir al dot 3), clic en el slide sin enlace hace scroll y deja la sección de categorías visible en pantalla.
- **Cabecera del Home rediseñada (2026-09-22, Fase 10, `page.tsx`):** el `<header>` sticky pasó de `bg-white/95` (franja blanca) a **oscuro** `bg-gray-900/95 backdrop-blur-md border-b border-white/10`, a juego con la barra fina superior (`bg-[#090d16]`, ya oscura desde antes). Estructura nueva en escritorio: `md:grid md:grid-cols-[1fr_auto_1fr]` con un spacer vacío a la izquierda (`hidden md:block`), el logo en la columna central (`md:justify-self-center` — geométricamente centrado, verificado en navegador: centro del logo = centro del header en 1366 px) y el buscador en la derecha (`md:justify-self-end`); en móvil sigue apilado (`flex flex-col`), con el logo también centrado (antes quedaba pegado a la izquierda). El logo ahora vive en una tarjeta blanca redondeada (`bg-white rounded-xl px-3 py-1.5 shadow-sm`) en vez de a bordes vivos: el PNG (`/images/logo-duna.png`) trae su propio fondo blanco de fábrica, invisible contra el header blanco anterior pero un rectángulo feo contra el nuevo fondo oscuro — la tarjeta lo convierte en un "badge" intencional. Buscador restilizado: `bg-white/10` (antes `bg-slate-100`), sin borde duro (`border-white/10`), texto e ícono claros (`text-gray-200`/`text-gray-400` en vez de `text-slate-800`/`text-slate-400`). **Selector de moneda restaurado:** el contenedor `Dual ($/Bs) / $ USD` en la barra fina superior **le faltaba por completo el tercer botón** (Bolívares) — no era un recorte de CSS, nunca se había escrito el `<button>`; se agregó `Bs. VES` (`onClick={() => setCurrencyMode('VES')}`, la lógica de `formatPriceBimonetary` ya soportaba `'VES'` desde antes, solo faltaba el botón) y se sumó `flex-wrap justify-center` + `whitespace-nowrap` al contenedor por si el texto no cabe en pantallas muy angostas. Verificado en navegador (1366 px y 390 px): los tres botones (`Dual ($/Bs)`, `$ USD`, `Bs. VES`) se ven completos en una sola fila en ambos tamaños, sin scroll horizontal.
