...
- Antes de implementar algo, revisa si ya existe una regla de negocio aquí que lo afecte
  (tasa BCV, logística, comandas; el Cofre Recompensa fue RETIRADO, ver §2.1). No reinventes ni contradigas estas reglas
...
### 2.1 Cofre Recompensa D'una (25% OFF en flete) — RETIRADO del frontend (2026-09-26, P0 Lote 2)

> **Estado actual: esta regla NO existe en el flujo real.** El simulador (`orderCount`, botón "Simular 3 compras", `usarRecompensa`, `descuentoUSD`) se eliminó de `CheckoutModal.tsx`: sin un contrato de backend que valide el descuento, el frontend no puede alterar el total a pagar (`totalFinalUSD = subtotalNeto + costoEnvio + propina`, `discountAmount` viaja 0). Solo `OrderTrackingModal` sigue mostrando el Cofre en pedidos ANTIGUOS guardados con `descuentoUSD`. Lo que sigue es histórico; reactivarlo exige un endpoint de fidelización real.
...
- **Flete dinámico real (2026-09-20):** `MerchantStoreView.tsx` cotiza cuando el carrito está
  ...
  tienda. `CheckoutModal` no envía el pedido si falta `location`. **Ubicación alterna (2026-09-20):** el pin de `CartModal` abre `LocationPickerModal`; al confirmar se reemplaza `customerLocation` (`label` = dirección del mapa) y todo lo demás (distancia, bloqueo >12 km, `getDeliveryRate`, `direccion`/`location` hacia `CheckoutModal`) se recalcula con el mismo efecto que usa el GPS. El botón "Nacional" solo se ofrece si la tienda declara `isNationalShippingEnabled` (P0 Lote 2); el checkout bloquea igualmente cualquier pedido nacional. `deliveryGps.ts` ahora delega en
...
- Pendiente de purga (no forman parte del flujo real): `tienda/demo-mostaza` pasa `bcvRate={40}` fijo y `nicheEngine="FOOD_FAST"`; [RESUELTO en el P0 Lote 2: `price || 1.5` eliminado, sin precio no se abre el modal ni se agrega al carrito]; `CheckoutModal` envía `discountAmount: "0"`/`totalWithoutDiscount` iguales al total aunque el Cofre descuente flete (sin verificar contra el backend, posible causa de `code: 21`).
...
- [RESUELTO en el P0 Lote 2] Ya no existe flete gratis por umbral (`esEnvioGratis` eliminado): `serviceAmount` es siempre la cotización oficial (0 solo en Pickup).
- [RESUELTO en el P0 Lote 2] El contador simulado del Cofre Recompensa se retiró del frontend (ver §2.1).
...
- `src/lib/deliveryGps.ts` y `src/services/marketplaceService.ts` son los únicos archivos con fallback de API key; ambos usan
  `bf8f1b64-…` (desde 2026-09-26 `src/app/page.tsx` ya no lo tiene: sus lecturas del Home pasan por el servicio).
...
 # #   2 0 2 6 - 0 9 - 2 4 :   C o r r e c c i � n   d e   A l i n e a c i � n   T a b u l a r   e n   R e c i b o   C o n t a b l e  
 -   R e f a c t o r i z a d a   l a   e s t r u c t u r a   d e   e x t r a s   e n   e l   t a b   R E C E I P T   u t i l i z a n d o   f l e x b o x   e s t r i c t o   c o n   a n c h o s   f i j o s   ( w - 2 4 ,   w - 1 6 ) .  
 -   A l i n e a c i � n   a   l a   d e r e c h a   f o r z a d a   p a r a   v a l o r e s   m o n e t a r i o s   g a r a n t i z a n d o   c o i n c i d e n c i a   v e r t i c a l   c o n   e l   t o t a l   y   s u b t o t a l .  
 -   P r e s e r v a d a   l a   i n t e g r i d a d   v i s u a l   d e   l a   p e s t a � a   K I T C H E N   ( c e r o   a l t e r a c i o n e s ) .  
 

## 2026-09-25: Pedido entre panas - monitor en vivo, pase a caja y experiencia del invitado

- **Anfitrion (`MasterProductModal.tsx`, viewMode `comboRoom`):** el monitor muestra "Ranuras ocupadas: X de Y" con una pastilla por ranura y, por participante, chip Listo/Eligiendo, exclusiones ("Sin ...") y adicionales, subtotal en USD y Bs. Sondeo cada 2 s a `/api/combo/[id]` (la sala vive en Firestore via API routes; no se usa `onSnapshot`). Pie: mientras faltan ranuras, boton "Compartir de nuevo por WhatsApp (x/y)"; al llenarse, el CTA "Proceder al Pago y Despacho (N/N)" llama `handleAddToCart(true)` -> payload con `proceedToCheckout: true` -> `MerchantStoreView.handleAddToCartFromModal` cierra el modal y abre el carrito (de ahi al `CheckoutModal`; NO se salta el carrito porque ahi se resuelven modo de entrega, ubicacion y cotizacion de flete reales). Se oculto el boton clasico "Agregar combo al carrito" en modo sala (sin duplicados). El total del pie en modo sala = suma de `subtotalUsd` de los participantes (lo mismo que se envia al carrito). `productCode` del item usa `product.code` real (antes `id`).
- **Invitado (`src/app/combo/[id]/page.tsx`):** carrusel horizontal de adicionales (solo opciones con precio real > 0 de grupos que no sean "SIN" ni pricingRole BASE), caja "Tu parte a pagar" con Bs. protagonista + "($X USD - Tasa BCV: Bs. Y)" (la pagina nunca cargaba la tasa: ahora usa `getBCVRate()`), `PUT` con `addonsUsd` (saneado en `api/combo/[id]/route.ts` y sumado a `subtotalUsd`), pantalla final "Tu pedido quedo reservado en la sala" con boton "Explorar el menu de la tienda" -> `/?store={code}`. `page.tsx` (Home) lee `?store=` una vez y abre esa tienda (hook ANTES del return condicional de la vista de tienda: un hook despues de ese return provoca React error #300).
- **Seguimiento (`page.tsx`):** un pedido colaborativo (`breakdown[0]` contiene "PEDIDO ENTRE PANAS") con comprobante o referencia adjuntos cierra el checkout y abre `OrderTrackingModal` (siempre arranca en la pestana Estatus). Sin pago adjunto se mantiene la confirmacion del checkout (ahi se elige como pagar) y su boton "Ver seguimiento".
- **Limitaciones conocidas:** (1) los adicionales de invitados viajan al carrito solo como texto en `breakdown`, no como `variants` estructuradas, y suben el total por encima del precio del combo: el backend recalcula por codigo de variante, posible `code: 21` (sin verificar contra el backend). (2) El carrusel de adicionales solo ofrece los adicionales reales del producto; la venta cruzada del catalogo de la tienda (papas/tequenos/refresco del modal) no esta disponible en la pagina del invitado porque la sala solo guarda `storeCode`. (3) La tasa BCV del invitado sale de `/api/bcv` (fuente del Home), no de `referenceRateValue` de la tienda. (4) No existen rutas por tienda: la salida del invitado usa el deep link `/?store=`.


## 2026-09-25 (2): Personalizar mi (unidad) del anfitrion y retorno seguro a la sala

- **Bug A/B (MasterProductModal.tsx):** el boton final de la vista de ranuras hacia siempre `setViewMode(options)`, asi que un anfitrion que personalizaba desde `host_setup` caia en la compra individual ("Agregar al carrito" con el total del producto) y perdia el contexto colaborativo. `slotReturnView` ahora es `options | host_setup | comboRoom` y se reinicia a `options` al entrar por las rutas individuales ("Personalizar combo" / "Personalizar aqui mismo"). El boton "Guardar personalizacion y volver a la sala" vuelve a `host_setup`, o desde el Monitor en Vivo hace `PATCH /api/combo/[id]` y regresa a `comboRoom`; hay un "Volver" para salir sin guardar.
- **Boton unificado:** "Personalizar mi {unidad}" en la tarjeta del anfitrion del monitor y en cada unidad de `host_setup`. `{unidad}` es la primera palabra del producto en minuscula (perro, combo, helado...), NO la palabra fija "perro": el modal sirve a cualquier producto. Terminologia: solo "Personalizar" (no se uso "Modificar").
- **Persistencia:** antes `createComboRoom` enviaba `hostSelectedVariants: {}` y lo personalizado en `host_setup` se perdia. Ahora `buildHostClaim` (exclusiones + opciones con cantidad + adicionales en USD) viaja en el POST (`hostAddonsUsd` suma al subtotal) y en el PATCH. `PATCH` solo admite `participantId: host`, no cambia `claimedUnits`, recalcula subtotal = unidades x precio unitario + adicionales. Igual que PUT, no autentica (cualquiera con el id de sala puede llamarlo) y lee-modifica-escribe el arreglo `participants` completo: dos escrituras simultaneas (host y un invitado) pueden pisarse; no se uso transaccion de Firestore.
- **Conocido, no tocado:** la etiqueta de precio de los chips de la vista de ranuras se ve como "(+$$2.50)" (doble signo de dolar, preexistente).


## 2026-09-25 (3): Notas de cocina compactas y fix de "$$"

- **Componente `KitchenNote.tsx`:** acordeon ultra-compacto. Cerrado: boton sutil "Alguna sugerencia para la cocina? (Opcional)". Abierto: input de una linea, `maxLength` 70, contador `n/70`. `cleanKitchenNote` recorta a 70 y cambia parentesis por corchetes (el Recibo de `OrderTrackingModal` detecta extras con el patron `(+X.XX)` y una nota como "poca salsa (+2)" se leeria como cobro).
- **Solo 2 puntos con notas (regla del negocio):** (A) cada unidad/ranura del combo, anfitrion (`ComboSlot.notes`, tras el carrusel de adicionales de la vista de ranuras) e invitado (un campo por reclamo en `combo/[id]/page.tsx`, aplica a todas sus unidades porque esa pagina no personaliza unidad por unidad); (B) el producto simple individual (`viewMode options`, no combo, no modo ranuras). **Prohibido** en Checkout y Delivery: no hay ningun campo de nota ahi.
- **Flujo de datos:** anfitrion -> `buildHostClaim().notes` (una por unidad, con prefijo `U2:` si son varias) -> POST `hostNotes` / PATCH `notes`; invitado -> PUT `notes`; la API los sanea (`cleanNotes`, 80 car., max 12) y los guarda en `participants[].notes`. Monitor: "N unidades - Con todo - Nota: ...". Al pasar a carrito, el desglose lleva `  >> NOTA: ...` por participante (mismo estilo que `>> EXTRA:`); en el simple, `Nota: ...` en `breakdown` y `notes` en el item; `cartItemId` incluye la nota (mismo producto con notas distintas no se fusiona). La Comanda POS imprime el desglose tal cual; el Recibo la ignora (no tiene precio).
- **LIMITACION CRITICA (pendiente con Osvaldo):** el contrato de `POST /delivery/request/purchase/web` (`DOCUMENTO_TECNICO_OSWALDO.md`) no define ningun campo de nota por item, y `CheckoutModal` no envia `notes`. Por eso las notas NO llegan al backend/comercio: solo se ven en el monitor, el carrito y la comanda/ticket local del cliente. Para que la cocina las reciba hay que acordar un campo por item (el prototipo huerfano `VariantModal.tsx` usaba `comments`) y agregarlo en el mapeo `itemsAdonis` de `CheckoutModal`. No se enviaron campos no documentados a un endpoint real de pedidos.
- **Fix estetico:** `(+$$2.50)` -> `(+$2.50)` en el chip de la vista de ranuras (una interpolacion `$${...}` en JSX escribia dos signos).


## Contrato Oficial de Backend (AdonisJS 5)

- **Fuente canónica de endpoints:** `DOCUMENTO_TECNICO_OSWALDO.md` (raíz del repo). Leerlo ANTES de tocar cualquier llamada a la API; nombres de campos, códigos de error y payloads se apegan a él (regla de oro 2).
- **Endpoint de compra:** `POST /delivery/request/purchase/web` (multipart: `orderData` JSON + `paymentFile` opcional; headers `apiKey` y `timeZone`).
- **Códigos de negocio (documentados):** `code: 1` éxito · `code: 15` validación de negocio (stock, comercio cerrado, datos inválidos) · `code: 21` inconsistencia de montos (`totalPaidDefaultAmount` = subtotalNeto + envío + propina en USD; `totalPaidReferenceAmount` = total × tasa BCV en Bs; también cupón/descuento).
- **Ítems (`orderData.data[]`), campos documentados:** `id, code, name, image, cant, pricing{unitBasePrice, addonsTotal, unitFinalPrice}, totalPrice, variants[], promo`.
- **Mapeo de notas de cocina por ítem: propiedad `comments`** en cada objeto de `orderData.data[]` (indicado por el equipo el 2026-09-25). **ADVERTENCIA de trazabilidad:** el `DOCUMENTO_TECNICO_OSWALDO.md` que hay en este repo (145 líneas, un solo commit `0c692f4`) NO contiene ese campo ni las secciones §6.2, §7.5, §8.1, §8.3, §10 ni §12.5 que se citan; solo describe el endpoint de compra, los códigos 1/15/21 y el ejemplo de payload. Las referencias a "§8.3" de este archivo (recálculo de precios por `code` de variante) tampoco están en esa copia. Hasta que se incorpore al repo la versión completa del documento, `comments` figura como mapeo acordado pero **no verificado por escrito**, y **el frontend aún NO lo envía** (`CheckoutModal` no manda `notes` ni `comments`; las notas de cocina hoy solo viven en el carrito, el monitor de sala y la Comanda POS local).
- **Riesgo `code: 21` con adicionales de la sala colaborativa (sin resolver):** los adicionales de los invitados hoy solo viajan como texto en `breakdown`, no como `variants` estructuradas con `pricing`, y elevan el total por encima del precio del combo. Si Adonis recalcula el total por catálogo, esos pedidos pueden fallar con `code: 21`. Pendiente de verificar con una orden real.


## 2026-09-25 (4): Contrato consolidado y `comments` por ítem (reemplaza la advertencia de la sección anterior)

- **`DOCUMENTO_TECNICO_OSWALDO.md` se reescribió en disco** con la versión consolidada que entregó el equipo (69 líneas; la anterior, con el ejemplo completo de payload —`service`, `location`, `paymentMethod`, `store`, etc.—, sigue en git en `0c692f4`). Ahora documenta: códigos 1/15/21, ítem con `comments`, `pricing` y `variants`, tarifa por distancia/duración y portal de comercio (`iac_store`). **Ya no lista los campos de nivel superior de `orderData`**: para eso, el payload real está en `CheckoutModal.tsx`.
- **`comments` YA se envía:** `CheckoutModal` agrega `comments` a cada ítem de `orderData.data[]` solo cuando el ítem trae nota (`item.notes`); los pedidos sin nota viajan exactamente igual que antes. Producto simple → su nota; combo con personalización por unidad → `#1 (Carlos): nota | #2: nota`; sala colaborativa → `Anfitrión: nota | Ana: nota`.
- **Adicionales de la sala estructurados (mitiga `code: 21`):** con `qty === 1` el ítem de sala viaja con `variants` (grupo real del producto: nombre/código/tipo, ítems con `quantity/unitPrice/totalPrice`) y `pricing` (`unitBasePrice` = total − adicionales, `addonsTotal`, `unitFinalPrice` = total de la sala). Los adicionales de host e invitados conservan `groupName/groupCode/groupType`. Con `qty > 1` (varios combos en la misma sala) no se puede representar por unidad y el ítem sigue sin estructurar: riesgo de `code: 21` sin resolver en ese caso. Las exclusiones ("SIN…") de invitados siguen solo como texto.
- **NO verificado en vivo:** no se ejerció `POST /delivery/request/purchase/web` (habría creado un pedido real). Se verificó en navegador la forma del ítem en el carrito. La primera orden real debe confirmar que Adonis acepta `comments` y que el total con adicionales cuadra.


## 2026-09-25 (5): Faro guiado reactivo en el flujo colaborativo (`MasterProductModal.tsx`)

- **Qué es:** un pulso breve que resalta el siguiente paso natural y hace scroll suave hacia él. React nativo + Tailwind, cero dependencias. Estado `activeBeacon` (`options | hostCustomize | shareWhatsApp | proceed | null`) con temporizador que lo apaga solo (2 s el hito 1, 2.5 s el resto); refs `modeOptionsRef`, `hostCardRef`, `launchBtnRef`, `proceedBtnRef`. `beaconScrollTo` usa `scrollIntoView({behavior:'smooth', block:'center'})` (`'nearest'` para los botones del pie sticky, ya visibles, para evitar saltos); con `prefers-reduced-motion` el scroll es instantáneo y el pulso no anima (`motion-safe:animate-pulse`).
- **Hitos:** (1) al entrar a la vista de modos, ring naranja en "Personalizar aquí mismo" y "Compartir entre panas por WhatsApp"; (2) en el setup de la sala, pulso naranja en "Personalizar mi {unidad}" si el anfitrión aún no personalizó; (3) si ya personalizó, o al regresar de personalizar, el pulso verde pasa a "Crear Sala y Enviar a WhatsApp"; (4) al llenarse la sala (Monitor en Vivo), foco (`focus({preventScroll:true})`) y pulso naranja en "Proceder al Pago y Despacho". Se dispara por cambio de `viewMode` (efecto con `prevViewModeRef`) y por `roomFull`.
- **Regla que no se debe romper:** estos hooks están ANTES del `if (!isOpen || !product) return null;`. Un hook después de ese return reproduciría el error #300 (mismo origen que el incidente de categorías del Home).
- **Además:** la tarjeta de unidad del anfitrión en el setup ahora dice "Personalizado" también cuando eligió opciones "SIN…" (precio 0) o dejó una nota (antes solo contaba exclusiones de texto y extras con precio).
- **Notas de la etiqueta:** el botón dice "Compartir entre panas por WhatsApp" (texto vigente en el código), no "Pedir entre panas…". No se tocaron `viewMode`, las notas/`comments` ni los cálculos bimonetarios.


## 2026-09-25 (6): Aislamiento del invitado, personalización por unidad, pie responsive y rescate de sala del anfitrión

- **Monitor (`MasterProductModal.tsx`):** se eliminó "Envía el link a tus panas para que se sumen" y "Copiar Link" (la invitación es solo por WhatsApp). En `md:` el botón "Compartir de nuevo por WhatsApp" es `md:w-auto md:px-6 md:py-3`, alineado a la derecha del pie (`md:flex md:justify-end`), sin tapar "TOTAL A PAGAR"; en móvil sigue a todo el ancho.
- **Invitado aislado:** `PedidoAmigosFloating` (montada en `layout.tsx`, global) devuelve `null` si `usePathname()` empieza por `/combo/`. El hook va antes de los returns condicionales. La ruta del invitado no tiene carrito ni drawer propios.
- **Invitado — complementos:** la sala guarda `storeId` (POST desde el modal, `store.id`). La página del invitado carga `getProductsByStore(storeId)` y, SOLO si el producto no trae adicionales con precio, ofrece complementos del catálogo con el mismo criterio del modal del anfitrión (papa/tequeño/bebida/refresco/extra/adicional/acompañante, sin `stock 0`/`outOfStock`, sin el propio producto, máx. 4; solo la primera página del catálogo). Viajan como adicionales de grupo `ADDONS` con `code = code||sku||id` del producto. **Sin verificar** si Adonis acepta productos del catálogo como ítems de variante `ADDONS` (el ejemplo del contrato usa ese nombre de grupo); pendiente de la primera orden real.
- **Invitado — varias unidades:** con `cant > 1` y "Quitar ingredientes": pestañas `#1…#n` con exclusiones independientes y "Repetir en todos" (copia la pestaña activa a todas). El PUT envía `unitExclusions` (lista por unidad, saneada en la API) además de `exclusions` (unión, para no romper lo que ya la lee). Monitor: "#1 Sin cebolla · #2 Con todo"; desglose de la comanda: una línea `  - #n: …` por unidad. Con `cant === 1` la vista es la de siempre. La nota de cocina sigue siendo una por reclamo. Las pestañas dicen "#1", no un emoji fijo de producto (el modal sirve a cualquier producto).
- **Rescate del anfitrión:** al crear la sala se guardan `localStorage['active_combo_host'] = {roomId, comboId, storeId}` y `duna_pedido_amigos_active` (con `isHost` y `productId`). La barra flotante muestra "Anfitrión · Pedido entre amigos activo"; **Volver** consulta la sala (`/api/combo/{id}`, fuente del `productId` y del slug) y navega a `/?store={slug}&resumeRoom={id}&resumeProduct={productId}` — nunca a `/combo/{id}` para el anfitrión. `MerchantStoreView` lee esos parámetros una vez al montar (los limpia con `history.replaceState(history.state, …)` para no pisar el estado de Next), abre el producto y pasa `resumeRoomId` a `MasterProductModal` (directo o vía `MerchantTemplateEngine`), cuyo efecto recupera la sala, restaura `hostSetupUnits`, reinicia el sondeo y deja `viewMode = 'comboRoom'`. Sala inexistente/vencida → se limpian ambas claves y queda el modal normal. Cancelar/completar borra ambas claves. **Limitación:** tras refrescar, la personalización local del anfitrión (`slots`) no se reconstruye; la sala conserva lo último guardado, y si vuelve a personalizar parte de cero (el PATCH reemplaza lo guardado).
- **No se tocó:** navegación circular monitor↔personalización, estructura del payload (`comments`, `pricing`, `variants`), lógica bimonetaria ni la posición de los hooks (todos los nuevos van antes de los `return` condicionales).



## 2026-09-25 (7): Vista del invitado unificada con "Personalizar aquí" + corrección del error 500 al unirse con varias unidades

- **Causa del 500 (error mío de la sección 6, ya corregido):** el `PUT /api/combo/[id]` guardaba `unitExclusions` como `string[][]`. **Firestore no admite arreglos anidados** (`Function updateDoc() called with invalid data. Nested arrays are not supported`), así que cualquier join con 2+ unidades personalizadas devolvía 500 ("Error al unirse a la sala"). Mis pruebas anteriores simulaban la API y no llegaban a Firestore. Reproducido y verificado ahora ejecutando el `route.ts` real (transpilado) contra la **validación real del SDK de Firestore** en un proyecto ficticio sin red: formato antiguo → error; formato nuevo → 200.
- **Nuevo modelo por unidad (`participants[].units[]`, solo mapas):** `{ unitIndex, unitName, exclusions[], addons[{name, code, price, count, groupName?, groupCode?, groupType?}], note }`. `cleanUnits()` sanea alias (30 car.), exclusiones (20×60), adicionales (20) y nota (70/80). `toFirestoreSafe()` limpia además `selectedVariants` en POST/PUT/PATCH (sin `undefined`; un arreglo dentro de un arreglo se envuelve en `{items}`), de modo que un cliente viejo no pueda volver a provocar el 500. El PUT acepta `unitsCount` o lo deduce de `units.length`, valida cupo (409 si no hay ranuras, no 500) y responde 200 con el estado de la sala (2 unidades en una sala de 5 con 1 ocupada → 3/5). Errores inesperados se registran en el servidor (`console.error`).
- **Vista del invitado (`/combo/[id]/page.tsx`), misma estructura que "Personalizar aquí":** Tu nombre ("Ej: Carlos, Nena…") + contador "¿Cuántos van para ti?" (tope = cupos libres). Con N > 1, pestañas `#1…#N` (muestran el alias) y "Repetir en todos" (copia exclusiones y adicionales, no alias ni nota). Cada unidad tiene: alias opcional ("¿Para quién es? (Opcional)", solo con N > 1), chips "Sin" (✓/+ como los del modal), carrusel "¿Acompañamos esta unidad?" con tarjetas del modal (adicionales reales del producto, o complementos de la primera página del catálogo de la tienda si no trae), y el mismo componente `KitchenNote`. Se eliminó el selector "Salen con todo / Quitar ingredientes": ahora las exclusiones se ven siempre, como en el modal; sin selección = "Con todo". Botón "Confirmar mi selección"; "TU PARTE A PAGAR" en Bs. grande + USD + tasa BCV (sin cambios); un solo mensaje de error amable (`role="alert"`), nunca el texto técnico del 500; sin barra flotante (oculta en `/combo/*`). Con 1 unidad no hay pestañas ni alias.
- **Consolidación hacia el anfitrión/Adonis (sin tocar la estructura de precios):** además de `units`, el invitado sigue enviando `exclusions` (unión), `selectedVariants.addons` (adicionales consolidados de todas sus unidades) y `addonsUsd`, que son lo que consume `pricing`/`variants` en el carrito. Con 1 unidad la nota viaja en `notes`; con varias, cada nota vive en su unidad.
- **Monitor, comanda y `comments`:** monitor "#1 (Nena) Con todo + Papas Fritas • Nota: Poca salsa · #2 (Carlitos) Sin salsa roja, Sin papita"; comanda una entrada por unidad (`- #1 (Nena): …`, `    + Papas`, `    >> NOTA: …`); `comments`: `Participante: #1 (Alias): Con todo • Nota: … | #2 (Otro): Sin salsa roja, Sin papita` (`formatSin` unifica "SIN SALSA ROJA" → "Sin salsa roja"). Participantes sin varias unidades siguen como antes.
- **No se tocó:** rescate de sesión del anfitrión (`active_combo_host`), `comments`/`pricing.addonsTotal`/`variants` hacia Adonis, lógica bimonetaria, navegación circular ni la posición de los hooks (todos los nuevos van antes de los `return` condicionales). **Sin verificar aún:** que Adonis acepte productos del catálogo como ítems del grupo `ADDONS` (pendiente de la primera orden real).


## 2026-09-25 (8): Recibo — beneficiario de cada adicional

- **Problema:** en pedidos de sala el Recibo listaba los adicionales con precio pero sin dueño (caían en el respaldo por `variants`, que no conoce al participante). Confirmado con una orden real: entró a Adonis sin `code: 21` y con la Comanda POS correcta.
- **Solución (solo presentación local):** el ítem que arma la sala al pasar al carrito (`MasterProductModal`, rama `comboRoom`) lleva un campo **local** `extrasByPerson: [{ name, participant, price }]` (precio ya multiplicado por cantidad; nombre con prefijo `Nx` si cantidad > 1). El beneficiario es: alias de la unidad (`units[].unitName`) si el invitado la nombró, si no el nombre del invitado, y `Anfitrión` para el anfitrión. Solo adicionales con costo; la suma coincide con `pricing.addonsTotal`. `MerchantStoreView` lo copia al ítem del carrito y `OrderTrackingModal` (pestaña Recibo) lo usa con prioridad sobre lo deducido del desglose/variantes: `PAPAS FRITAS (Carlos)   +5.10`, el nombre inline y el precio a la derecha. Sin el campo (pedidos anteriores u otros flujos) el Recibo se comporta exactamente como antes.
- **Compatibilidad con Adonis (`DOCUMENTO_TECNICO_OSWALDO.md`):** el campo NO viaja a Adonis: `CheckoutModal` mapea campos explícitos del ítem (`id, code, name, image, comments, cant, pricing, totalPrice, variants, promo`), así que `pricing.addonsTotal`, `variants`, `comments` y los totales no cambian. Persiste porque `onFinalizeOrder` guarda `items` completos en `localStorage[last_active_order]`, que es de donde lee el Recibo.
- **Límite conocido:** los adicionales que el anfitrión elige por unidad se consolidan en una sola lista (no distingue el alias que puso en su propia ranura), por eso siguen rotulados `(Anfitrión)`.

- **Faro Guiado del checkout (2026-09-25, `src/lib/beacon.ts`, `CartModal.tsx`, `CheckoutModal.tsx`)**: hook `useBeacon` (pulso naranja `#FE6712` que se apaga solo + `scrollIntoView` suave; no pelea con el scroll manual: se omite si el usuario scrolleo hace menos de 1.5 s, y usa scroll instantaneo con `prefers-reduced-motion`). CartModal: sin ubicacion en delivery (o cotizacion bloqueada) pulsa la tarjeta de ubicacion; cotizacion lista pulsa "PROCEDER AL PAGO". CheckoutModal: elegir metodo pulsa "CONTINUAR AL PAGO"; al entrar a la Fase 3 (la tarjeta bancaria solo existe ahi) scroll a los datos y pulso en "Copiar Todos Los Datos"; al copiar, scroll y pulso en la referencia; con referencia >= 4 caracteres o comprobante, pulso en el boton principal. Todos los hooks van antes de los `return null` (regla #300). Sin cambios en payload de Adonis, bimonetario ni flujo colaborativo. Nota: `CartV2` no existe; los componentes reales son `CartModal` y `CheckoutModal`. Detalle en `docs/technical/bitacora-arquitectura-farmacia-storefront.md`.

- **order_number y tipos numericos (2026-09-25)**: `OrderTrackingModal` (cabecera, Comanda POS, Recibo) y `/order/[orderId]/timeline` muestran `order_number` con respaldo al `id` (antes al reves). `CheckoutModal` sanea `orderData.data[]`: `cant` entero y `totalPrice`/`pricing.*` como number con 2 decimales. (Actualizado: la confirmacion del checkout ya muestra el numero de orden; ver la entrada "Numero de pedido visible".)

- **Sanitizacion numerica de `variants[]` (2026-09-25, `src/components/CheckoutModal.tsx`)**: motivo: el carrito acumula flotantes (2.1 x 3 = 6.300000000000001) y cantidades/montos podian llegar como string; el contrato de Adonis exige `Number`. Solucion: helper `sanitizeVariantNumbers` aplicado a cada grupo de `variants[]` al armar `orderData.data[]`: `items[].quantity` = `parseInt(String(q || 1), 10)`, `items[].unitPrice` / `items[].totalPrice` = `parseFloat(Number(x || 0).toFixed(2))` (si falta `totalPrice` se deriva de unitario x cantidad) y `selected.unitPrice` igual. Solo se tipan esos campos; nombres y estructura del contrato no cambian. `money()` devuelve 0 ante NaN. Los campos de nivel superior (`serviceAmount`, `tip`, `totalPaidDefaultAmount`, `totalPaidReferenceAmount`, `distance`, `duration`, `discountAmount`) estaban como string al registrar esta entrada; desde el 2026-09-25 viajan como `number` (ver entrada "Campos raiz numericos"). Verificado en navegador con el POST de compra interceptado y abortado (nunca llego al backend). Proceso: `.next/` esta versionado y `npm run build` dentro del repo lo modifica; verificar el build en una copia aislada o restaurar con `git checkout -- .next && git clean -fd .next`. Nunca commitear `.next/` ni `tsconfig.tsbuildinfo`.

- **Restauracion visual y UX (2026-09-25, `MasterProductModal.tsx`, `KitchenNote.tsx`)**: motivo: en moviles el pie fijo (Total a pagar + boton) dejaba el ultimo campo/nota sin espacio; medido a 360x640, el contenedor de opciones de la vista estandar (productos sin variantes y medicamentos, `flex-1 overflow-y-auto`) se colapsaba a ~24 px y la nota quedaba atrapada en un mini-scroll pegado al pie. Solucion: (1) ese contenedor ahora es `flex-1 shrink-0 p-3 sm:p-4 pb-32 sm:pb-32 md:pb-6 space-y-4` (sin scroll propio, scrollea el contenedor padre; colchon `pb-32` en movil y `pb-6` en md+ para no estirar el modal en escritorio); la vista con variantes ya traia `pb-32` en la columna derecha. (2) La nota es universal: etiqueta "¿Alguna nota o indicacion especial? (Opcional)" y placeholder "Escribe aqui cualquier detalle, preferencia o indicacion..."; ya no depende del nicho (se muestra en farmacias, bodegones, ferreterias y restaurantes, en productos simples no-combo, en cada ranura del combo y en la vista del invitado) y pasa de input de 1 linea a textarea de 2 lineas (tope 70, contador) para que el placeholder se lea completo a 360 px; su texto sigue viajando en `notes` -> `comments` del item (verificado con el POST interceptado en un producto de farmacia; el item sin nota no lleva la clave). Reemplaza la etiqueta "sugerencia para la cocina" de la entrada 2026-09-25 (3). (3) Se elimino el bloque muerto `(false) &&` "Panel Colaborativo" (boton "Copiar" + WhatsApp duplicado) y sus helpers `comboCopied`, `handleCopyComboLink`, `whatsappComboUrl`; el Monitor en Vivo ya no tenia "Copiar Link" ni "Envia el link..." desde la entrada 2026-09-25 (6). (4) El boton "Compartir de nuevo por WhatsApp" ya era `md:w-auto md:px-6 md:py-3` (327 de 720 px a 1366 px, alineado a la derecha sobre "Total a Pagar"); el CTA "Proceder al Pago y Despacho" (sala llena) sigue a todo el ancho. (5) La ruta del invitado `/combo/[id]` ya ocultaba la barra flotante (`PedidoAmigosFloating` con `usePathname`) y no tiene carrito propio: verificado con la sesion del anfitrion y un carrito guardado en el mismo navegador (sin pastilla, sin carrito, 0 elementos `position: fixed`, "Confirmar mi seleccion" completo y encima de todo al fondo a 360x640). Sin cambios en `orderData`/`comments` ni hooks nuevos. Detalle en la bitacora tecnica.

- **Campos raiz numericos en `orderData` (2026-09-25, `src/components/CheckoutModal.tsx`)**: motivo: Oswaldo confirmo que los campos numericos de nivel superior deben viajar como `number`, jamas como string (antes: `"1.90"`, `"5.40"`, `"4"`, `"0"`). Solucion: en `osvaldoPayload`, `duration`, `distance`, `serviceAmount`, `tip`, `totalPaidDefaultAmount`, `totalPaidReferenceAmount`, `discountAmount` y `totalWithoutDiscount` pasan por el helper `money()` (`parseFloat(Number(x || 0).toFixed(2))`, devuelve 0 ante NaN; el mismo de los items y `variants[]`). Solo cambia el tipo: los redondeos son los de siempre (minutos enteros y distancia a 1 decimal; `MerchantStoreView` los redondea antes de llamar a `getDeliveryRate`, asi la cotizacion y el pedido no se desfasan). `totalWithoutDiscount` no estaba en la lista de Oswaldo; se convirtio tambien por ser el gemelo de `totalPaidDefaultAmount` (mismo valor sin descuento) y no dejar tipos mezclados: si el backend lo quiere como string, revertir esa sola linea. Siguen siendo string a proposito: `durationText`, `distanceText`, `foodStoreId`, `couponId`, `couponCode`, `phone`, `customerName`, `customerDocument`, `paymentRef`, `address`, `vehicleType`, `service`, `ftoken`. Las 26 claves y su orden no cambian. Verificado en produccion (copia aislada) con el POST interceptado y abortado: en delivery (1.9 km: duration 4, distance 1.9, serviceAmount 1.9, tip 0.5, totales 5.4 y 4627.85) y en pickup (todo 0, `PICKUP`) todos son `number`, sin NaN, con suma coherente (items + flete + propina = total; total x tasa = Bs). NO se ejercito el envio real a Adonis: la primera orden real debe confirmar que acepta los tipos. Detalle en la bitacora tecnica.

- **Numero de pedido visible = `order_number` y botones del pie acotados en desktop (2026-09-25; `CheckoutModal.tsx`, `OrderTrackingModal.tsx`, `order/[orderId]/timeline/page.tsx`, `MasterProductModal.tsx`)**: (2.1) motivo: requisito de Adonis: el cliente y el comercio ven el correlativo comercial (`order_number`), no el id primario de la base de datos. Solucion: `CheckoutModal` toma `response.data.order_number` (o `orderNumber`) y lo pasa en `onFinalizeOrder({ orderNumber })` (campo opcional nuevo de `SubmittedOrderPayload`; se persiste en `last_active_order` solo si es un correlativo real, el id nunca se guarda como numero). Las cabeceras de las confirmaciones dicen "Confirmacion De Orden · N° X" / "Orden registrada · N° X" (el correlativo se resuelve desde el seguimiento publico: ver la entrada 2026-09-26 "Resolucion del order_number"; ya no cae al id). `OrderTrackingModal` y `/order/[orderId]/timeline` calculan `displayId = remote.order_number || orderData.orderNumber || id (respaldo)` y NO muestran el id mientras carga la primera consulta (la cabecera dice "Orden" y luego "Orden #N", para que no aparezca un instante el id que despues cambia). Comanda POS ("COMANDA N° X") y Recibo ("#X") usan el mismo valor. El id primario sigue siendo el que se usa para consultar (`getOrderPublic`), para `last_active_order_id` y para la URL `/order/{id}/timeline`. [Superado el 2026-09-26] Se confirmo que la respuesta de compra solo trae `{ id, url }`: el correlativo se obtiene del endpoint publico (ver la entrada "Resolucion del order_number"). (2.2) motivo: en `md:` los botones principales del pie del modal de producto/sala se estiraban al 100 %. Solucion: "Crear Sala y Enviar a WhatsApp", "Compartir de nuevo por WhatsApp" y "Proceder al Pago y Despacho" pasan a `md:w-auto md:px-8`, alineados a la derecha; en la sala (`comboRoom`) el pie es una sola fila en md+ (`md:flex md:flex-row-reverse md:items-center md:gap-6`): Total a Pagar a la izquierda y el boton a la derecha; en movil (<md) el orden apilado y el ancho completo no cambian. Medido en navegador: a 1366/1024/768 px el boton mide ~45 % del pie (24 px de margen derecho) y el Total queda a 24 px del borde izquierdo en la misma fila; a 360 px el boton sigue a todo el ancho sobre el Total. Sin hooks nuevos y sin cambios en `orderData`. Detalle en la bitacora tecnica.

- **Resolucion del order_number via seguimiento publico + verificacion de tipos numericos (2026-09-26, `src/components/CheckoutModal.tsx`)**: contexto (documento tecnico de Oswaldo, §8.2/§9.2): `POST /delivery/request/purchase/web` responde SOLO `{ code: 1, data: { id, url } }` y NO devuelve `order_number`; el correlativo comercial (ej. 1724) sale unicamente de `GET /delivery/request/{id}/public` (aqui `getOrderPublic`; el `getOrderTimeline` del documento). (1) Solucion: tras `code: 1`, `CheckoutModal` lanza en segundo plano `fetchCommercialNumber(id)` (helper de modulo: hasta 4 consultas, esperas de 0.7, 1.4 y 2.1 s; acepta `code: 1` con `data.order_number`); al llegar, `setNumeroOrden` pinta "· N° X" en la cabecera de la confirmacion y se persiste `orderNumber` en `last_active_order` (si su `id` coincide) para que el seguimiento arranque ya con el correlativo. Si no llega, la confirmacion queda SIN numero: nunca se muestra el id primario (antes de este cambio caia al id). `latestOrderIdRef` (hook al nivel superior, antes del `return null`) descarta el resultado si entre tanto se creo otro pedido. `OrderTrackingModal` y `/order/[orderId]/timeline` no cambian: `order_number` del endpoint publico primero y el id solo como respaldo cuando la consulta ya termino (error o sin `order_number`). (2) Tipos numericos: `CartV2.jsx` y `purchaseNow()` NO existen en este repo; el unico camino que arma y envia la compra es `CheckoutModal.tsx` (`handleCompleteFinalOrder` -> `submitPurchaseOrder`, unico llamador). Desde `d3bf6f7`, `13f2c28` y `a22622f` (ya en `origin/main`) raiz, items, `pricing` y `variants[]` salen como `number`; verificado otra vez en el build de produccion, con el POST interceptado y abortado, incluso con un carrito "sucio" (strings y flotantes): 0 cadenas numericas, sin NaN, mismas 26 claves. Un cURL con strings corresponde a un build anterior a esos commits o a codigo ajeno a este repo: comprobar que el deploy los incluya. Verificado en navegador (POST simulado con solo `{ id, url }`): con el endpoint publico bien, la confirmacion pasa de "Confirmacion De Orden" a "· N° 1724" y el seguimiento abre con "#1724" (0 apariciones del id 1620); con el endpoint caido no aparece ningun numero ni el id; con 2 respuestas "no encontrado" acierta en la 3ª consulta. Detalle en la bitacora tecnica.

- **P0 Lote 1 — aislamiento de carritos, ciclo de vida del checkout, sin mocks y exito estricto (2026-09-26; `src/lib/cartStorage.ts` nuevo, `src/app/page.tsx`, `src/components/MerchantStoreView.tsx`, `src/components/CheckoutModal.tsx`, `src/services/marketplaceService.ts`; auditoria completa en `docs/technical/AUDITORIA_PREPRODUCCION_2026.md`)**: (C1) `localStorage['cart_data']` ya NO es un arreglo: es `{ storeId, items }` y toda lectura/escritura pasa por `src/lib/cartStorage.ts` (`readCart(storeId)` devuelve los items solo si el `storeId` coincide EXACTAMENTE; `writeCart` guarda siempre con la tienda y elimina la clave si el carrito queda vacio; `clearCart`; `purgeCartIfOtherStore`). Cualquier codigo que escriba `cart_data` a mano (p. ej. la nota de `SalesRecoveryAssistant`) debe usar `writeCart`. `MerchantStoreView` inicializa el carrito con `readCart(merchant.id)` y persiste con `writeCart`; `handleStoreClick` purga el carrito de otra tienda, del formato viejo (arreglo suelto) o con JSON roto SOLO cuando la tienda va a abrirse. `current_cart_store_id` se conserva pero ya no es la defensa. (C2) `page.tsx` monta `<CheckoutModal key={checkoutKey}>`: `checkoutKey` se incrementa al cerrar el ciclo de un pedido (`handleCloseCheckout` con pedido creado, `onViewTracking`, pedido colaborativo que salta al seguimiento) y al abrir una tienda; en los mismos puntos se apaga `hasCompletedOrder`, asi cerrar el checkout de un pedido NUEVO ya no borra su carrito ni cierra la tienda. Tambien `isCheckoutOpen` se apaga al salir de la tienda (`popstate`/`onBack`) y al abrir otra: antes el checkout viejo reaparecia sobre la siguiente tienda. `CheckoutModal` ademas reinicia sus estados de pedido (`pasoVista`, `ordenCreada`, `ordenId`, `numeroOrden`, `pagoPendiente`, `referenciaPago`, comprobante...) si abre con `ordenCreada` (red de seguridad; hook antes del `return null`); nombre, cedula y telefono se conservan. (C3) Eliminados los fallbacks ficticios de `page.tsx` (tiendas 991/992 `isDemo` y productos "Combo de Prueba" y similares): sin datos reales NO se muestra nada falso. `marketplaceService.apiFetch` acepta un `timeoutMs` opcional con `AbortController` real; `REQUEST_TIMEOUT_MS = 15000` se aplica a `getProductCategories`, `findStores` (nuevos, reemplazan los `fetch` directos del Home; `page.tsx` ya no define `API_BASE/API_KEY/TIMEZONE`), `getProductsByStore` y `getStorePromotions`. Home: `loadHomeData` (categorias en paralelo pero sin bloquear, promociones no bloquean) y, si las tiendas fallan, vista de error con "Reintentar". `handleStoreClick` ya no usa `Promise.race` de 3 s: espera hasta 15 s con overlay "Abriendo <tienda>...", guardia anti doble clic (`openingStoreRef`) y, si no hay catalogo real, NO abre la tienda y muestra un banner con "Reintentar"/"Cerrar". (C4) `submitPurchaseOrder` solo acepta un sobre JSON: texto plano/HTML/vacio (aunque el HTTP sea 200/201) devuelve `code: 0` con mensaje claro (el estado HTTP ya no se convierte en `code`), un sobre sin `code` numerico se normaliza a `code: 0`, y el fallo de red devuelve un mensaje en espanol. `CheckoutModal` considera la compra exitosa UNICAMENTE con `response.code === 1` y `data.id` real (no vacio ni 0): se elimino el UUID generado en el cliente y la aceptacion de `code` 200/201; `code: 1` sin `data.id` se trata como fallo con aviso de comunicarse con el comercio antes de reintentar. Sin cambios en `orderData` ni en los tipos numericos. Pendiente (Lote 2, RESUELTO el 2026-09-26: ver la entrada "P0 Lote 2" mas abajo): C5 envio gratis por umbral, C6 Cofre simulado, C7 modo Nacional, C8 timeout de la compra e idempotencia, C9 mensajes de `code` 15/21; `uploadPaymentReference` y `apiFetch` conservan el mapeo HTTP->`code` para respuestas no JSON (forma de exito del PUT sin verificar); `MerchantStoreView` mantiene el precio de respaldo `price || 1.5` del detalle de producto (no es un producto ficticio, pero es un dato inventado: revisar). Verificado en navegador (build de produccion aislado, POST interceptado/simulado): C1 (Papa Helado -> Franela sin carrito; misma tienda restaura; formato viejo y JSON roto se purgan), C2 (segundo pedido en la misma sesion abre el formulario, el carrito no se borra, se completan 2 pedidos; variante con "Atras" del navegador), C3 (API lenta de 3.5 s = 47 tiendas reales; API caida = error + "Reintentar"; API colgada = error a los ~15 s; catalogo caido = la tienda no abre y no hay productos falsos), C4 (HTTP 200 no JSON, `code:1` sin id, `code:201`, `id:0` y red caida = fallo con mensaje claro, sin confirmacion, sin pedido guardado y con el carrito intacto; exito real = id real del backend). Detalle en la bitacora tecnica.
- **P0 Lote 2 — saneamiento financiero, timeouts y codigos de negocio (2026-09-26; `src/services/marketplaceService.ts`, `src/components/MerchantStoreView.tsx`, `src/components/CartModal.tsx`, `src/components/CheckoutModal.tsx`; auditoria en `docs/technical/AUDITORIA_PREPRODUCCION_2026.md`)**: (C5) se ELIMINO el "envio gratis desde $20": `MerchantStoreView` ya no calcula `esEnvioGratis`/`progresoEnvio`/`faltaParaEnvioGratis`/`discountDelivery` ni tiene el estado `rewardMode`; el flete es siempre la cotizacion oficial (`quote.rate` de `getDeliveryRate`) y vale 0 solo mientras no hay cotizacion (en ese caso `deliveryBlocked` deshabilita "PROCEDER AL PAGO" y el flete se muestra "—"); total = subtotal + flete (Pickup = 0). `CartModal` perdio las props de recompensa, el bloque "Barra de Recompensa Inteligente" y el badge "100% OFF". Verificado: un carrito de $25 (antes gratis) cobra $1.90 y el payload lleva `serviceAmount: 1.9` y `totalPaidDefaultAmount: 27.4` (= items + flete + propina). (C6) el Cofre Recompensa simulado se RETIRO del frontend: en `CheckoutModal` desaparecen `orderCount`, `usarRecompensa`, el boton "Simular 3 compras", el bloque visual y `descuentoUSD`; `totalFinalUSD = subtotalNeto + costoEnvio + propina` y `discountAmount` viaja 0 (`OrderTrackingModal` sigue mostrando el Cofre solo para pedidos ANTIGUOS guardados con `descuentoUSD`). Sin soporte de backend el frontend no altera el total a pagar. Tambien se elimino el precio de respaldo `price || 1.5`: la tarjeta muestra "Ver precio" si el listado no trae precio; si el detalle falla y el listado tampoco trae precio, no se abre el modal (aviso "No pudimos cargar la informacion de este producto"); `handleAddToCartFromModal` bloquea cualquier item con `totalPrice <= 0`. (C7) `MerchantStoreView` pasa `isNationalShippingEnabled={Boolean(merchant?.isNationalShippingEnabled)}` (antes `true`); `CartModal` tiene ese prop en `false` por defecto y el boton "Nacional" no se dibuja si la tienda no lo soporta; un efecto devuelve el modo a `delivery` si estaba en `national` sin soporte; `CheckoutModal` (`handleProceedToInstructions` y `handleCompleteFinalOrder`) bloquea cualquier pedido `national` con aviso, asi que nunca viaja como `DELIVERY` local con flete fijo inventado (aun si una tienda lo habilitara, el checkout lo bloquea hasta que exista contrato de backend). Ninguna tienda real de DEV declara envio nacional: solo se ofrecen Delivery y Pickup. (C8) `submitPurchaseOrder` usa `AbortController` con `PURCHASE_TIMEOUT_MS = 25000`; `ApiResponse` suma `errorKind` (`timeout` / `network` / `invalid_response`) = RESULTADO AMBIGUO (el pedido pudo haberse creado; una respuesta JSON con `code` es un fallo definitivo). `CheckoutModal`: `submittingRef` (guardia sincrona contra doble toque, ademas del estado `submitting`), el boton pasa de "Registrando tu pedido..." a "Ya verifiqué, reintentar pedido" al vencer, aviso "No sabemos si {comercio} llego a recibirlo. Antes de intentarlo de nuevo, verifica con el comercio para no duplicar tu pedido" y enlace `wa.me` "Verificar con {comercio} por WhatsApp" (`toWhatsAppNumber(merchantPhone)`). El contrato no tiene llave de idempotencia: por eso un resultado ambiguo se resuelve con verificacion humana, no con reintento automatico. (C9) `classifyPurchaseFailure()`: `code 15` (o tienda cerrada/sin jornada/sin stock/sin saldo) -> mensaje en espanol con el motivo comercial: si el backend manda texto natural se muestra tal cual tras "{comercio} no pudo recibir tu pedido:"; los codigos tipo `E_STORE_NOT_OPEN` se traducen ("esta cerrado o no tiene una jornada activa..."); sin mensaje, texto generico; errores tecnicos (HTTP 500, stacks, `E_*`) NUNCA se muestran crudos. `code 21` (`E_AMOUNT_MISMATCH`) -> `handleAmountMismatch()`: vuelve a la Fase 2 (`setPasoVista('formulario')`), refresca en segundo plano `getStorePaymentInfo` (tasa y metodos, conservando el metodo elegido) y `getDeliveryRate` (estado `fleteRefrescado`, `costoEnvio = fleteRefrescado ?? orderSummary.costoEnvio`) y muestra un aviso ambar (`avisoTarifa`) con lo que cambio ("tasa Bs. 857.01 -> Bs. 865.58 y flete $1.90 -> $2.10"); el reintento ya usa los valores nuevos. Verificado en navegador (build de produccion aislado, POST de compra SIEMPRE interceptado/simulado, GPS simulado; `tsc` y `npm run build` exit 0): C5 ($25 cobra $1.90, sin texto de envio gratis; el unico "Gratis" en pantalla es la descripcion real de un producto del backend), C6 (sin "Cofre"/"Simular"/"Usar Ahora"; `discountAmount: 0`; precio `null` -> "Ver precio", detalle caido = aviso sin modal, sin precio = aviso y carrito vacio), C7 (solo botones Delivery y Pickup), C8 (peticion colgada 45 s -> error a los 25.08 s con 1 solo POST; triple clic simultaneo = 1 POST; red caida = mismo aviso con enlace de WhatsApp), C9 (7 variantes de `code` 15/HTTP 500 con mensaje correcto y sin texto tecnico; `code 21` -> Fase 2 con aviso y total $5.60 / Bs. 4.847,25; el reintento envia `serviceAmount: 2.1`, `totalPaidDefaultAmount: 5.6`, `totalPaidReferenceAmount: 4847.25` = 5.6 x 865.58). Regresion OK: C1-C4 del Lote 1, tipos numericos del payload y resolucion de `order_number`. Limites conocidos: `uploadPaymentReference` sigue sin timeout ni `errorKind`; `apiFetch` general sin timeout salvo opt-in (`timeoutMs`); las respuestas `code 15/21` se SIMULARON, asi que el texto real de `message` de Adonis (y si el recalculo de 21 coincide con el refresco) queda por confirmar con la primera orden real; `CartDrawer` y el codigo de envio gratis de `MerchantTemplateEngine` (sin uso) no se tocaron. Detalle en la bitacora tecnica.
