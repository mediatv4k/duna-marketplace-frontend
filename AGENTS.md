...
- **Orden del listado de tiendas (Home):** prioridad **estrictamente por horario en tiempo real** (`scheduleStatus`): 1° `OPEN`, 2° `OPENING`, 3° `CLOSED`/`INACTIVE`/sin horario activo (el `status` administrativo ya no cuenta); orden estable (dentro de cada grupo se respeta el del backend). Una tienda con `status: OPEN` y `scheduleStatus: OPENING` (p. ej. Papá Helado, "Abre a las 12:00 PM") queda en el grupo 2. La píldora de horario de cada tarjeta (texto de `scheduleInfo`) es un botón que abre `StoreScheduleModal` (con `stopPropagation`, no abre la tienda). **Cabecera de la tienda (`MerchantStoreView`):** el badge de estatus/horario `merchant.badge` (= `scheduleInfo`, p. ej. "Abre a las 12:00 PM"; naranja, junto al chip de categoría) volvió a renderizarse y ahora es un botón que abre el mismo `StoreScheduleModal`. Nota histórica: en `b8411a7` ese badge era un `span` no clicable; el botón de reloj "Ver Horarios" con modal vivía en la tarjeta del Home.
- **Botón "Rastrear Pedido" del header (Home):** restaurado (original de `b8411a7`, eliminado en `b1edf0f`): botón naranja con reloj, solo `md:` en adelante (`hidden md:flex`, como el original; en móvil se usa el FAB), que abre `OrderTrackingModal` con `setIsTrackingOpen(true)`. No se restauró la píldora "Flota D'una Activa • 14 en ruta" que lo acompañaba porque era un dato inventado.
