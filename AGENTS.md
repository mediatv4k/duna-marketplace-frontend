...
| `OrderTimelinePanel.tsx` | Vista de seguimiento **compartida** por la pestaña Estatus de `OrderTrackingModal` y la página `/order/[orderId]/timeline`: tarjeta de cliente + estado actual, ficha del repartidor (`wa.me`), botón "📍 Ver en Google Maps" y timeline descendente. Recibe `remote` (data de `/public`). |
...
| `orderTracking.ts` | Helpers del seguimiento: `FINAL_STATUSES`/`isFinalStatus`, `friendlyStatus` (nombres amigables), `parseCoords` (string JSON `{latitude,longitude}` o `{lat,lng}`), `toWhatsAppNumber` (`04…` → `58…`). Usado por el modal, el panel, la página pública y el FAB. || `shareUtils.ts` | Utilidades de compartir (ej. generación de texto/enlace de pedido). |
