...
//  · núcleo universal: Navbar corporativo (Wallet / BCV), Drawer del carrito con termómetro gamificado y MasterProductModal;
          ...
          <div className="flex items-center gap-2 shrink-0">
            {!!bcvRate && bcvRate > 0 && (
              <span className="hidden sm:flex items-center gap-1.5 rounded-full bg-slate-100 px-3 py-1.5 text-[11px] font-bold text-slate-600">
                <Coins className="w-3.5 h-3.5 text-amber-500" /> BCV: <strong className="text-slate-900">Bs. {bcvRate.toFixed(2)}</strong>
              </span>
            )}
            {walletBalanceUSD !== null && walletBalanceUSD !== undefined && (
              <span className="flex items-center gap-1.5 rounded-full bg-emerald-50 border border-emerald-200 px-3 py-1.5 text-[11px] font-black text-emerald-700">
                <Wallet className="w-3.5 h-3.5" /> ${walletBalanceUSD.toFixed(2)}
              </span>
            )}
            <button
              type="button"
              ...
              className="relative flex h-9 w-9 items-center justify-center rounded-full bg-[#fe6712] text-white shadow-md cursor-pointer hover:bg-[#e0580d] transition"
            >
              <ShoppingBag className="w-4 h-4" />
              {totalItems > 0 && (
                <span className="absolute -top-1 -right-1 flex h-4 min-w-4 items-center justify-center rounded-full bg-slate-900 px-1 text-[9px] font-black text-white">
                  {totalItems}
                </span>
              )}
            </button>
