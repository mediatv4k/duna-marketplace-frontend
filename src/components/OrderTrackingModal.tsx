...
export default function OrderTrackingModal({ isOpen, onClose, orderId, orderSummary }: OrderTrackingModalProps) {
  ...
  // Correlativo comercial (`order_number`) para el cliente, la comanda y el recibo. El id primario solo como respaldo, y no mientras
  // se hace la primera consulta (así no se ve un instante el id que luego cambia por el correlativo). El id sigue siendo el que
  // se usa para consultar el seguimiento (`resolvedId`) y para la URL /order/{id}/timeline.
  const commercialNumber = remote?.order_number || orderData?.orderNumber || '';
  const displayId = commercialNumber || ((remote || trackingError) ? (remote?.id || orderData?.id || orderId || '') : '');
      ...
      <div className="w-full max-w-md my-auto max-h-[90dvh] flex flex-col rounded-3xl overflow-hidden shadow-2xl bg-white border border-slate-100">
        ...
        <div className="bg-[#fe6712] px-5 py-3 text-white flex items-center justify-between shrink-0">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h2 className="text-sm font-black tracking-tight leading-none">Orden{displayId ? ` #${displayId}` : ''}</h2>
