...
export default function MasterProductModal({
  ...
  useEffect(() => {
    if (isOpen) {
      setDisplayImage(product?.image || product?.img || '');
    }
  }, [isOpen, product]);
  ...
  const handleRedesign = async () => {
    ...
    try {
      const res = await fetch('/api/admin/recompose-image', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ imageUrl: displayImage, productId: product.id })
      });
      const data = await res.json();
      if (data.success && data.recomposedUrl) {
        setDisplayImage(data.recomposedUrl);
        // Toast opcional en una implementación real
        console.log("Imagen rediseñada exitosamente con IA");
      } else {
      ...
      console.error(err);
      alert('Falló el rediseño con IA');
    } finally {
                ...
                <div className={`grid grid-cols-1 ${viewMode === 'slots' ? '' : 'md:grid-cols-2'} gap-4 md:gap-6 p-4 md:p-6 items-start`}>
                  ...
                  <div className={`w-full flex flex-col justify-between overflow-hidden bg-slate-50/70 rounded-2xl p-4 border border-slate-200/80 gap-3 md:sticky md:top-6 ${viewMode === 'slots' ? 'hidden md:hidden' : ''}`}>
                    ...
                    <div className="relative flex items-center justify-center w-full aspect-square max-h-72 rounded-2xl border border-slate-200/80 bg-white overflow-hidden p-3 shrink-0 shadow-xs">
                      <img
                        src={getOptimizedImageUrl(displayImage, 'PRODUCT')}
                        alt={product.name}
                    ...
                    <button 
                      ...
                      disabled={isRedesigning || !displayImage}
                      className="w-full flex items-center justify-center gap-1.5 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 text-sm font-semibold px-4 py-2 rounded-xl transition-all disabled:opacity-50 border border-indigo-200"
                    >
                      {isRedesigning ? (
                        <>
                          ...
                          Recomponiendo imagen con IA...
                        </>
                      ) : (
                        '✨ Rediseñar a 512x512 con IA'
                      )}
                    </button>
                ...
                <div className="shrink-0 p-3 sm:p-4 grid grid-cols-1 sm:grid-cols-2 gap-3 items-start bg-white z-20 shadow-sm border-b border-slate-100">
                  ...
                  <div className="flex flex-col gap-2.5">
                    <div className="relative flex items-center justify-center w-full aspect-square max-h-72 rounded-xl border border-slate-200/80 bg-white overflow-hidden p-3">
                      <img src={getOptimizedImageUrl(displayImage, 'PRODUCT')} alt={product.name} className="w-full h-full object-contain" onError={(e:any)=>{e.target.src='https://images.unsplash.com/photo-1542838132-92c53300491e?w=500&auto=format&fit=crop&q=60'}} />
                    </div>
                    <button 
                      onClick={handleRedesign}
                      disabled={isRedesigning || !displayImage}
                      className="w-full flex items-center justify-center gap-1.5 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 text-[11px] font-semibold px-2 py-1.5 rounded-lg transition-all disabled:opacity-50 border border-indigo-200"
                    >
                      {isRedesigning ? (
                        <>
                          <svg className="animate-spin -ml-1 mr-1.5 h-3.5 w-3.5 text-indigo-700" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
                          Recomponiendo con IA...
                        </>
                      ) : (
                        '✨ Rediseñar a 512x512 con IA'
                      )}
